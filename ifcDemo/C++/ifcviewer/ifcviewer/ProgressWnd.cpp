...
CProgressWnd::CProgressWnd(CWnd* pParent)
{
	CommonConstruct();
...
BOOL CProgressWnd::Create(CWnd* pParent)
{
	BOOL	bSuccess;
...
void CProgressWnd::SetText(LPCTSTR fmt, ...)
{
	...
	va_start(args, fmt);
	_vstprintf(buffer, 255, fmt, args);
	va_end(args);
...
void	CProgressWnd::GetPreviousSettings()
{
	int	x = AfxGetApp()->GetProfileInt(szSection, szEntryX, -1),
		y = AfxGetApp()->GetProfileInt(szSection, szEntryY, -1);
